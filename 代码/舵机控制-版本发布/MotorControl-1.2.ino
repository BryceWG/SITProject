...
float ultrasonicDistance = 0.0; // 超声波距离变量
...
float calculateDeltaTime(unsigned long *last_time); // 计算时间差

void moveCaculate(void); // 读取MPU6050传感器数据，并进行角度估算
void readMPUData(sensors_event_t *a, sensors_event_t *g, sensors_event_t *temp); // 读取MPU6050传感器数据
...
void readDisplacement(void); // 读取小车位移
...
void loop()
{
    ...
    else if (frontObstacle == 0 && distanceLimit == 1)   // 行驶距离到达上限
    {
        readUltrasonicStand();
        distanceLimit = 0;
        first_call = true;
...
void moveForward()  // 向前移动
{
    ...
    static float target_distance = 1000; // 目标前进距离，单位：mm
    ...
    distanceLimitDetect(float initial_displacement_x, float initial_displacement_y);
}
...
void moveCaculate(void)  // 读取MPU6050传感器数据，并进行角度估算
{
    ...
    readMPUData(&a, &g, &temp);

    static unsigned long last_time = 0; // 记录上一次调用的时间
    static float velocity_x = 0, velocity_y = 0; // 记录速度
    float dt = calculateDeltaTime(&last_time);  // 计算时间差
    ...
    //计算姿态角    
    float roll = getRoll(a.acceleration.x, a.acceleration.y, a.acceleration.z);
    float pitch = getPitch(a.acceleration.x, a.acceleration.y, a.acceleration.z);
...
void readUltrasonicStand(void)   //超声波雷达扫描函数
{
...
void distanceLimitDetect(float initial_displacement_x, float initial_displacement_y)   //超声波距离上限检测函数
{
    ...
    if (current_distance >= 5000)     //行驶距离超过5000mm时，停止
    {
...
{
    Read_Motor_V(); // 读取电机速度
    PID_Cal_Computer_Out();// 调用PID计算函数，计算PID输出并更新电机PWM值
}
...
void setupInit(void)  // 初始化函数，包括初始化MPU6050、超声波雷达、PID控制器、电机控制引脚
{
    mpuInit();
    ultrasonicInit();
    PID_Init();
