...
// 简单低通滤波器参数
const float alpha = 0.5; // 滤波系数，0 < alpha < 1
    ...
    while (Serial2.available() > 0) 
    {
        ...
        if (yawIndex != -1) 
        {
            ...
            if (!isnan(parsedYaw)) 
            {
                ...
                Serial.print(PREFIX_SENSOR + "Parsed Yaw: ");
                Serial.println(yaw);
            } 
            else 
            {
                Serial.println(PREFIX_ERROR + "Error: Parsed Yaw is NaN.");
            }
        } 
        else 
        {
