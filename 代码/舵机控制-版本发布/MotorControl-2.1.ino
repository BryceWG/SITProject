...
float angleDifference(float target, float current); // 计算两个角度之间的差值
...
void readMagnetometerYaw() {
    while (Serial2.available() > 0) {
        ...
        if (yawIndex != -1) {
            ...
            yawString.trim();
            float parsedYaw = yawString.toFloat();
            if (!isnan(parsedYaw)) {
                magYaw = parsedYaw;
                Serial.print("Parsed Yaw: ");
                Serial.println(magYaw);
            } else {
                Serial.println("Error: Parsed Yaw is NaN.");
            }
        } else {
            Serial.println("Error: Yaw data not found in magnetometer output.");
        }
    }
}
...
void setup()// Arduino的setup函数，用于初始化
{
    ...
    Serial2.begin(115200); // 初始化Serial2，用于与磁力计通信
...
void moveForward()  // 向前移动
{
    ...
    {
        initial_angle = magYaw; // 记录初始角度
        initial_displacement_x = x; // 记录初始位移
        initial_displacement_y = y;
        first_call = false; // 标记为非第一次调用
    }

    float angle_offset = angleDifference(initial_angle, magYaw); // 计算角度偏差
...
void turn(float turnAngle)  // 角度转向
{
    ...
    float startAngle = magYaw;   // 记录当前角度
    ...
    while (abs(angleDifference(targetAngle, magYaw)) > 1)    // 当当前角度与目标角度差值大于1时，持续转向
    {
        if (turnAngle < 0) {
            // 左转
...
{
    // 步骤1：预测
    rateKF[axis] = newRate - biasKF[axis];
    angleKF[axis] += dt * rateKF[axis];
    ...
    // 步骤2：更新
    float y_err = newAngle - angleKF[axis];
    float S = P[axis][0][0] + R_measure;
    float K[2] = {P[axis][0][0] / S, P[axis][1][0] / S};

    angleKF[axis] += K[0] * y_err;
    biasKF[axis] += K[1] * y_err;
...
void obstacleDetectStand(void)      //静止时障碍物检测函数
{
    ...
    if (obstacleDistanceCheck(30.0))    //当距离小于30cm时，认为有障碍物
    {
        ...
        Serial.println("left obstacle exist\n");
    }
    else
    {
        leftObstacle = 0;
    ...
    if (obstacleDistanceCheck(30.0))    //当距离小于30cm时，认为有障碍物
    {
        ...
        Serial.println("right obstacle exist\n");
    }
    else
    {
        rightObstacle = 0;  
