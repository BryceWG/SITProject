...
try:
    ...
    # 设置 Matplotlib 使用支持中文的默认字体（例如，SimHei）
    matplotlib.rcParams['font.sans-serif'] = ['SimHei']  # 替换为系统中存在的中文字体
        ...
        try:
            if ser.in_waiting:
                ...
                while start_index != -1 and end_index != -1:
                    data_block = buffer[start_index + len('<START>'):end_index]
                    buffer = buffer[end_index + len('<END>'):]  # 移除已处理的数据

                    # 处理数据块
                    lines = data_block.strip().split('\n')
                    for line in lines:
                        # 提取位置数据
                        match_position = pattern_position.match(line)
                        if match_position:
                            ...
                            continue
                        # 提取扫描数据
                        match_scan = pattern_scan.match(line)
                        if match_scan:
