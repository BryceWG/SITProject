...
import math   #导入数学库
import serial  #导入串口库
import settings #导入设置文件settings.py
#变量的初始化
angle = 0
distance = 0.0
point_x = 0
...
size = settings.width,settings.height   #设置窗口大小
screen = pygame.display.set_mode(size)  #显示窗口
pygame.display.set_caption("超声波雷达监测")    #设置窗口标题
...
while True:
    ...
    screen.fill(settings.bg_color)  #填充背景色
    ...
    for points in all_points: #遍历不同点集
        for point_info in points:   #遍历点集中的点
            x, y, point_angle, point_distance = point_info  #获取点的信息
            rect_point = (x, y, settings.point_width, settings.point_width) #获取点的矩形
            ...
            # 显示该点的角度和距离信息
                info_text = f'Angle:{point_angle}°, Distance:{point_distance}cm'    #显示的信息
                text_surface = font.render(info_text, True, settings.text_color)    #渲染文字
                screen.blit(text_surface, (mouse_x + 10, mouse_y))  #显示文字

        if len(points) > 1: #当点集中的点大于1时
            for i in range(1,len(points)):  #遍历点集中的点
    ...
    #绘制UI字块
    if distance < 45:   #当距离小于45cm时
        ftob_surf = font.render("In Range",True, settings.text_color)   #物体在范围内
        ftdi_surf = font.render("Current Distance: "+str(distance)+"cm", True, settings.text_color) #显示当前距离
    else:
        ftob_surf = font.render("Out of Range",True, settings.text_color)   #物体在范围外
        ftdi_surf = font.render("Current Distance:   cm",True, settings.text_color) #显示当前距离
    ftan_surf = font.render("Current Angle: "+str(angle)+"°",True, settings.text_color) #显示当前角度
