...
#include <Adafruit_MPU6050.h>   // 引入MPU6050库，用于读取MPU6050传感器数据
...
void turn(float turnAngle)  // 角度转向
{
    ...
    while (abs(angle - targetAngle) > 1)    // 当当前角度与目标角度差值大于1时，持续转向
    {
        if (turnAngle < 0) {
            // 左转
...
void calibrateMPU6050(void) // 校准MPU6050传感器，计算加速度计和陀螺仪的偏移量
{
    ...
    Serial.print("accelXoffset: "); Serial.println(accelXoffset, 3); 
    Serial.print("accelYoffset: "); Serial.println(accelYoffset, 3);  
    Serial.print("accelZoffset: "); Serial.println(accelZoffset, 3);  
    Serial.print("gyroXoffset: "); Serial.println(gyroXoffset, 3);    
    Serial.print("gyroYoffset: "); Serial.println(gyroYoffset, 3);   
...
void moveCaculate(void)  // 读取MPU6050传感器数据，并进行角度估算
{
    ...
    a.acceleration.z -= accelZoffset;   // 减去偏移量，以获得更准确的读数
    ...
    dt = (currentTime - prevTime) / 1000.0; // 计算时间差
