...
def import_data(filename: str) -> pd.DataFrame:
    ...
    # 定义需要转换为数值的列
    numeric_columns = ['Timestamp_ms', 'k_p', 'k_i', 'k_d', 'TargetSpeed(mm/s)', 'ActualSpeed(mm/s)', 'Error(mm/s)']
    ...
    data = data.rename(columns={
        'TargetSpeed(mm/s)': 'TargetSpeed_mm_s',
        'ActualSpeed(mm/s)': 'ActualSpeed_mm_s',
        'Error(mm/s)': 'Error_mm_s'
    })
...
def add_param_id(data: pd.DataFrame, control_type: str) -> pd.DataFrame:
    ...
    print(f"控制类型: {control_type}, 参数组合数: {len(params)}, ParamID 范围: {params['ParamID'].min()} - {params['ParamID'].max()}")
...
def analyze_performance(data: pd.DataFrame, param_id: int, phase: str, control_type: str) -> dict:
    ...
    # 提取控制参数
    param_row = data[data['ParamID'] == param_id].iloc[0]
    k_p = param_row['k_p']
    k_i = param_row['k_i']
    k_d = param_row['k_d']
...
def plot_performance_comparison(performance_df: pd.DataFrame):
    ...
    # 绘制超调量与稳态误差的关系图
    plt.figure(figsize=(12, 8))
    markers = {'PID': 'o', 'PI': 's', 'P': '^'}  # 可以根据需要添加更多控制类型
    for ctl_type in performance_df['ControlType'].unique():
        df = performance_df[performance_df['ControlType'] == ctl_type]
...
def main():
    ...
    """
    # ---- 您可以在此处调整自定义参数值 ----
    filename = 'code_1_data_1.csv'  # 请替换为您的数据文件名
    # ----------------------------------------

    # 定义要分析的运动阶段
    motion_phases_to_analyze = ['加速', '减速', '静止', '突然加速', '突然停止']
