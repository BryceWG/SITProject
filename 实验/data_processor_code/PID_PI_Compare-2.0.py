...
def comprehensive_ranking(performance_df: pd.DataFrame, phase_weights: dict) -> pd.DataFrame:
    ...
    # 按照 ParamID、ControlType 分组，计算综合得分
    total_scores = df.groupby(['ParamID', 'ControlType', 'k_p', 'k_i', 'k_d']).agg({
        'WeightedPerformanceScore': 'sum'
    }).reset_index()

    # 如果某些参数组合的 WeightedPerformanceScore 为 0，则可能是因为其所有阶段的 PhaseWeight 都为 0，需剔除
    total_scores = total_scores[total_scores['WeightedPerformanceScore'] > 0]

    # 按综合得分排序
    total_scores = total_scores.sort_values('WeightedPerformanceScore').reset_index(drop=True)
